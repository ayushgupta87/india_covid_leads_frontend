...
Future Validate() async {
  ...
  var getinfo = await http.get(currentCustomerURI, headers: {
  ...
  if (getinfo.statusCode == 200) {
    if (jsonDecode(getinfo.body)['admin_approval'] == 0) {
      print('Not Approved by admin');
      return null;
    }
  ...
  if (messageIs == 'Token has expired') {
    var refreshInfo = await http.post(customerRefreshTokenURI, headers: {
